...
PostgreSQL + Prisma is the live store (`PrismaStore` in `app/repositories/prisma.ts`). Live web and worker hydrate from Postgres and write Shop, Order, Snapshot, Evaluation, Finding, Outbox, Usage, and related rows with `shopId` on every persist. Demo and automated tests use the shipped `MemoryStore`, which enforces the same unique keys, tenant scoping, and compare-and-swap `rowVersion` rules. Redis/BullMQ is the intended live job backend; demo and tests use an in-process job table on `MemoryStore`.
